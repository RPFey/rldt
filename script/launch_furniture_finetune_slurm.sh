#!/bin/bash
# Launch multiple slurm jobs for furniture-bench finetune configs.
#
# Usage:
#   ./script/launch_furniture_finetune_slurm.sh --commit COMMIT --exp EXPNAME [--task TASK] [--method METHOD] [--dry-run] [EXTRA_ARGS...]
#
# Arguments:
...
#   --task TASK       - filter by task; exact match (e.g. "lamp_low", "one_leg_low";
#                       "all" or omitted matches everything)
#   --method METHOD   - filter by config name; exact match (e.g. "ft_ppo_diffusion_unet";
#                       "all" or omitted matches everything)
...
#
# Example:
#   ./script/launch_furniture_finetune_slurm.sh --commit abc1234 --exp ppo-run
#   ./script/launch_furniture_finetune_slurm.sh --commit abc1234 --exp ppo-run --task lamp
#   ./script/launch_furniture_finetune_slurm.sh --commit abc1234 --exp ppo-run --task lamp_low --method ft_ppo --dry-run train.n_epochs=10
...
SLURM_SCRIPT="${ws}/script/furniture_slurm.sh"
...
# ---------------------------------------------------------------------------
# Step 2: enumerate all task config files under cfg/furniture/finetune
# ---------------------------------------------------------------------------
FINETUNE_CFG_DIR="${WORKTREE_DIR}/cfg/furniture/finetune"
...
echo "Found ${#CONFIG_FILES[@]} config(s) under cfg/furniture/finetune:"
...
for cfg_path in "${CONFIG_FILES[@]}"; do
    # cfg_path: .../cfg/furniture/finetune/<task>/<config_name>.yaml
    CONFIG_DIR=$(dirname "${cfg_path#${WORKTREE_DIR}/}")   # e.g. cfg/furniture/finetune/one_leg_low
    CONFIG_NAME=$(basename "$cfg_path" .yaml)               # e.g. ft_ppo_diffusion_unet
    TASK=$(basename "$CONFIG_DIR")                          # e.g. one_leg_low

    # Apply filters: "all" passes everything; otherwise exact match
    [[ "$TASK_FILTER"   != "all" && "$TASK"        != "$TASK_FILTER"   ]] && continue
