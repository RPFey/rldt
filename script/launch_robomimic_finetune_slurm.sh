#!/bin/bash
# Launch multiple slurm jobs for robomimic finetune configs.
#
# Usage:
#   ./script/launch_robomimic_finetune_slurm.sh --commit COMMIT --exp EXPNAME [--task TASK] [--method METHOD] [--dry-run] [EXTRA_ARGS...]
#
# Arguments:
...
#   --task TASK       - filter by task; exact match (e.g. "can", "lift", "square", "transport";
#                       "all" or omitted matches can, lift, square, transport)
...
#
# Example:
#   ./script/launch_robomimic_finetune_slurm.sh --commit abc1234 --exp ppo-run
#   ./script/launch_robomimic_finetune_slurm.sh --commit abc1234 --exp ppo-run --task can
#   ./script/launch_robomimic_finetune_slurm.sh --commit abc1234 --exp ppo-run --task square --method ft_ppo --dry-run train.n_epochs=10
...
# ---------------------------------------------------------------------------
# Step 2: enumerate all task config files under cfg/robomimic/finetune
# ---------------------------------------------------------------------------
FINETUNE_CFG_DIR="${WORKTREE_DIR}/cfg/robomimic/finetune"
...
echo "Found ${#CONFIG_FILES[@]} config(s) under cfg/robomimic/finetune:"
...
for cfg_path in "${CONFIG_FILES[@]}"; do
    ...
    # cfg_path: .../cfg/robomimic/finetune/<task>/<config_name>.yaml
    CONFIG_DIR=$(dirname "${cfg_path#${WORKTREE_DIR}/}")   # e.g. cfg/robomimic/finetune/can
    CONFIG_NAME=$(basename "$cfg_path" .yaml)               # e.g. ft_ppo_diffusion_mlp
    TASK=$(basename "$CONFIG_DIR")                          # e.g. can

    # Apply filters: "all" matches the four standard robomimic tasks; otherwise exact match
    if [[ "$TASK_FILTER" == "all" ]]; then
        [[ "$TASK" == "can" || "$TASK" == "lift" || "$TASK" == "square" || "$TASK" == "transport" ]] || continue
    else
